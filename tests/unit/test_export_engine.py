...
# Profil versionné avec le dépôt plutôt que cherché sur la machine :
# découvert dynamiquement, il manquait sur tout runner de CI et ces tests
# se sautaient en silence derrière un vert trompeur.
from tests.conftest import CMYK_PROFILE as _CMYK_PROFILE
...
def test_black_goes_through_the_profile_not_the_naive_formula(
    export_engine, black_pdf_path, output_dir
):
    """The naive conversion gave C184 M172 J171 N225 — roughly equal inks, the
    signature of `C = 255 - R`. Through a real profile the black is generated by
    the press condition instead, whatever ink load that condition calls for."""
    settings = JobSettings(export_format="TIFF", export_dpi=150,
                           icc_profile_path=str(_CMYK_PROFILE))
    result = export_engine.export_sheet(uuid4(), Sheet(job_id=uuid4(), sheet_number=1),
    ...
    # Le dosage dépend de la condition d'impression visée (100K sous SWOP, noir
    # riche à 330 % sous FOGRA39) : figer un seuil ici rendait le test tributaire
    # du profil installé sur la machine. Ce qui doit être vrai quel que soit le
    # profil : le noir est porté par le canal N et ce n'est pas la formule naïve.
    naive = Image.new("RGB", (2, 2), (0, 0, 0)).convert("CMYK").getpixel((0, 0))
    assert k > 200, f"le noir doit porter sur le canal N (N={k})"
    assert (c, m, y, k) != naive, "la conversion ne doit pas être la formule naïve"
