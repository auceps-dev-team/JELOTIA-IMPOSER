...
def _app_version() -> str:
    ...
    Resolution order:
    1. importlib.metadata — works when the package is installed via ``pip install -e .``.
    2. src._version — always bundled in the PyInstaller build; generated by bump_version.py.
    3. pyproject.toml — last resort for bare ``uv run`` without editable install.
    """
